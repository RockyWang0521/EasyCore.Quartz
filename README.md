...
> **EasyCore.Quartz** 是面向 .NET 8 的生产级任务调度封装库。基于 [Quartz.NET](https://www.quartz-scheduler.net/)，提供特性驱动任务、Hangfire 风格英文 Dashboard、REST 管理接口、HTTP 动态任务，以及 MySQL / SQL Server / PostgreSQL / Oracle 持久化与集群能力。
...
| 缺少可视化运维面 | 内嵌英文 Dashboard（`/easy-quartz`） |
...
| 包名 | 职责 | 是否必须 |
|---|---|---|
| `EasyCore.Quartz` | 核心、Dashboard、REST、History | ✅ |
...
打开：`http://localhost:<port>/easy-quartz`
...
| `HistoryCapacity` | 进程内历史条数（默认 200） |
...
> ⚠️ History 为**进程内环形缓冲**，不是跨节点共享审计库。
...
{
    options.Authorization.Add(new LocalRequestsOnlyAuthorizationFilter());
    // 生产：实现 IEasyCoreQuartzAuthorizationFilter 校验登录/角色
    // Authorization 为空 ⇒ 拒绝所有请求（默认安全）
});
```

| 过滤器 | 场景 |
|---|---|
| `LocalRequestsOnlyAuthorizationFilter` | 本机开发 |
| `AllowAllAuthorizationFilter` | 仅内网受控环境（公网禁用） |
| 自定义 `IEasyCoreQuartzAuthorizationFilter` | 生产推荐 |
...
共用代码：[`demo/WebApp.Quartz.Shared`](demo/WebApp.Quartz.Shared)
...
- [ ] Dashboard 使用真实鉴权过滤器（公网禁用 `AllowAll`）
...
**Q: Dashboard 打开是 401？**  
A: 未配置鉴权过滤器时默认拒绝。开发环境添加 `LocalRequestsOnlyAuthorizationFilter`。

**Q: 内存模式有 Dashboard 吗？**  
A: 有。9.0 起管理能力与是否持久化无关。
